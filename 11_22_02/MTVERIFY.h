...
__inline void PrintError(LPCSTR linedesc, LPCSTR filename, int lineno, DWORD errnum)
{
	LPSTR lpBuffer;
	WCHAR errbuf[256];
#ifdef _WINDOWS
...
#ifndef _WINDOWS
	WriteFile(GetStdHandle(STD_ERROR_HANDLE), errbuf, wcslen(errbuf), &numread, FALSE );
	Sleep(3000);
#else
